  ...
  <div class="card-container">
    ...
    <div class="infinite-card">
        <div class="image-container">
          <div class="image-surface" :style="backGroundImage" @click.stop="ShowEvent(event.id)"></div>
        </div>
        <div class="info-container">
          ...
          <p class="description">{{event.brief_description | truncate(120)}} </p>

          <div class="btn-actions">
            <span class="card-btn more-info" style="cursor: pointer" @click.stop="ShowEvent(event.id)">More Info</span>
...
export default {
  ...
  methods: {
    ...
    AddEventToCalendar(calType) {
      ...
      if (calType === 'iCal' || calType === 'Outlook') {
        ...
        window.open(`/calendar?title=${encodeURIComponent(this.event.title)}&description=${encodeURIComponent(this.event.brief_description)}&location=${encodeURIComponent(this.event.address)}&time_start=${encodeURIComponent(this.event.date_times[0].start_time)}&time_end=${encodeURIComponent(this.event.date_times[0].end_time)}`)
      } else if (calType === 'Google Calendar') {
        var time_start_formatted = moment(this.event.date_times[0].start_time).format('YYYYMMDDTHHmmss')
        var time_end_formatted = moment(this.event.date_times[0].end_time).format('YYYYMMDDTHHmmss')
  ...
  mounted: function(){
    console.log('card loaded:', this.event)
  },
  computed:{
    backGroundImage: function(){
      return 'background: url(\''+this.event.image+'\') center center / cover no-repeat; cursor: pointer;'
    },
    venue_info: function() {
      return []
      ...
      let current_venue = all_venues.find((venue) => {
        return venue.id === this.event.venue_id
      })
